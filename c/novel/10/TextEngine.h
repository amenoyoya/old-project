...
public:
	...
	TextEngine(): status(0), is_running(false) {}
	...
	void run(){
		...
		if(is_running){
			// 処理が終わったらrunning不可能にしてstatusを1進める
...
void engine_set_message(uint_t proc_no, _lpcSTR str){
	if(g_text_engine.status == proc_no) g_text_engine.set(str);
}
