...
void init_Back_Surface()
{
	...
	Load_Bmp( Back_DC,"Back00.bmp");		//デフォルトの背景を読み込んでおく	
}
...
{	
	...
	//BitBlt(Back_DC,500,0,25,25,Parts_DC,25,0,SRCAND  );	//AND
	//BitBlt(Back_DC,500,0,25,25,Parts_DC, 0,0,SRCPAINT);	//OR		
	...
	switch(msg){
		...
		case	WM_KEYDOWN:	//キーが押された
					switch(wprm){
							...
							case	VK_RETURN:
								//カーソルブリンク中ならブリンク解除
								if( Mode_stat.flag_cursor_blink == ON ) Com_cursor_blink_end();
								break;
					}
					break;
