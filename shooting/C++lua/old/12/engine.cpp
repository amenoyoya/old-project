...
void load_story(DxLibEngine &){
	File file("../dat/csv/storyH01.csv");
...
// 敵行動パターン配列
void (*enemy_pattern[_EnemyPatternMax])(int) = {
	...
	enemy_pattern9, enemy_pattern10
};
...
void act_enemy(){
	for(int i=0;i<_EnemyMax; ++i){
		if(enemy[i].flag==1){//その敵のフラグがオンになってたら
			if(0 <= enemy[i].pattern
				&& enemy[i].pattern < _EnemyPatternMax)
			{
				...
				//敵が画面から外れたら消す
				if(enemy[i].x<-20 || _FieldMaxX+20<enemy[i].x
					|| enemy[i].y<-20 || _FieldMaxY+20<enemy[i].y) enemy[i].flag=0;
			}else{
				printfDx("enemy[i].patternの%d値が不正です。",
					enemy[i].pattern);
			}
		}
	}
